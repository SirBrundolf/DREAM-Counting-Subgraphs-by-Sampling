...
def sample_centered(g, edges):
    #e = np.random.default_rng().choice(edges, 1, p=lambda_list)
    e = random.choices(edges, weights=lambda_list, k=1)
    u, v = e[0][0], e[0][1]
    ...
    if len(neighbors_of_u_bigger_than_v) == 0 or len(neighbors_of_v_bigger_than_u) == 0:
        return sample_centered(g, edges)
    else:
